...
current_met_DMA = read.table(here::here("data/met_DMA.txt"), header = TRUE,  sep = '\t')
...
# merge the old and new data sets and save

updated_met_DMA = merge(current_met_DMA, new_met_DMA)

write.table(updated_met_DMA,
            file = here::here("data/met_DMA.txt"),
            row.names = FALSE,
            quote = FALSE,
            sep = '\t',
            overwrite = TRUE)
