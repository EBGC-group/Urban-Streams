...
During the process of analyzing certain logger data streams we will need meteorological data such as barometeric pressure, rainfall, air temperature, etc. This is particularly useful to converting pressure transducer readings to stream depth or for calculating the saturation point of dissolved gases such as oxygen.
...
met_DMA = read.table(here::here("data/met_DMA.txt"), header = TRUE,  sep = '\t')
```
...
write.table(met_DMA, file = here::here("data/met_DMA.txt"), row.names = FALSE, quote = FALSE, sep = '\t')
```

After this original file is written the file is updated automatically every month through a github action: `get-merge-met-data.yml`. This action calls the script, `get-merge-met-data.R` to dowload new data and merge it into the met_DMA.txt file.
